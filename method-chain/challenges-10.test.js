...
const totalSum = (input) => {
  let sumCounter = 0;
  input.forEach(innerArr => {
    let innerSumArr = innerArr.reduce((previousVal, currentVal, currentIndex, array) => {
      return previousVal+=currentVal;
    },0)
    //console.log(`inner sum: ${innerSum}`);
    sumCounter += innerSumArr;
  });
  return sumCounter;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let returnArr = [];
  input.forEach(innerArr => {
    returnArr.push((innerArr.filter(element => typeof(element) === 'number' && element %5 ===0)).map(element => Math.pow(2, element)));
  });
  return returnArr;
};
...
let findMaleAndFemale = (data) => {
  let filteredArr = data.filter(character => character.gender === 'female' || character.gender === 'male');
  let returnString = '';
  for (let i=0; i<filteredArr.length; i++){
    let characterName = filteredArr[i].name;
    returnString = returnString.concat(characterName);
...
let findShortest = (data) => {
  let arrNames = data.map(element => [element.name, element.name.length]);
  arrNames.sort((a,b)=>{return a[1]-b[1]});
