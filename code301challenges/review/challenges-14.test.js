...
const toTitleCase = (arr => {
  return arr.map (element => {
    return `${element.charAt(0).toUpperCase()}${element.substr(1, element.length)}`
  });
});
...
let biggerThanLuke = (arr) => {
  const lukesMass =77;
  let charactersBiggerThanLuke = arr.reduce ((accumulator, character)=>{
    let nameString = '';
    if (parseInt(character.mass) > lukesMass){
      nameString = ` - ${character.name}`;
    }
    return `${accumulator}${nameString}`;
  },'')

  return charactersBiggerThanLuke.substr(3, charactersBiggerThanLuke.length);
};
...
const detectTicTacToeWin = (board) => {
  ...
  function checkForThree(whichThree) {
    //console.log('hello');
    for (let i=0; i<3; i++){
      console.log('which', whichThree[i]);
      console.log('board', board[whichThree[i]]);
    }
  }
