...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(element => {
    if (Object.values(element)[0] === character) {
      if (Object.values(element)[2].length > 0){trueFalse = true;}
...
const hasChildrenEntries = (arr, character) => {
  let trueFalse = 0;
  arr.forEach(element => {
    let characterName = Object.entries(element)[0][1];
    let numberChildren = Object.entries(element)[2][1].length;
    if (characterName === character) {
      if (numberChildren > 0) {trueFalse = true;}
      else {trueFalse = false}
    }
  });
  return trueFalse;
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(element => {
    let numberAdults = 1;
    let numberChildren = Object.values(element)[2].length;
    if (Object.values(element)[1] !== null) {
      numberAdults = 2;
    }
    let numberInFamily = numberAdults + numberChildren;
    //console.log(numberInFamily);
    totalCharacters = totalCharacters + numberInFamily;
  });
