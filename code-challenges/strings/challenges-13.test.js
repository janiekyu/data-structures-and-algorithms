...
const findHappiness = (arr) => {
  return arr.filter(element => {
    //console.log(`${element} ${element.includes(':)')}`)
...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.forEach(element => {
    let newPhoneNumber = element.replace(/\(|\) |-/g, '');
    newPhoneNumberArr.push(newPhoneNumber);
  });
...
const onlyOddChars = (str) => {
  ...
  for (let i=0; i<str.length; i++){
    if (i%2 !== 0){oddString = oddString.concat(str.charAt(i));}
  }
  return oddString;
};
...
const allHappy = (arr) => {
  let happyArr = arr.filter(element => {
    return element.includes(':)');
  });
  if (happyArr.length === arr.length){return true}
  else {return false}
};
...
const findAnything = (arr, target) => {
  let includesArr = arr.filter(element => {
...
const findEvery = (arr, target) => {
  ...
  if (includesArr.length === arr.length){return true}
  else {return false}
};
...
const unenrollBrook = (arr) => {
  ...
  arr.forEach(innerArr => {
    let newRowArr = [];
    newRowArr = innerArr.filter(element => {
      return !element.includes('Brook');
    })
    newRosterArr.push(newRowArr);
  });
  return newRosterArr;
};
